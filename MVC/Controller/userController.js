...
export const getAllUser = async (req, res) => {
  try {
    ...
    if (
      password == process.env.AdminLoginPass &&
      adminName == process.env.AdminName
    ) {
      const allUsers = await userModel.find();
      res.status(200).json(allUsers);
    } else if (adminName == process.env.AdminName) {
      res.status(500).send("password is incorrect");
    } else if (adminName == process.env.AdminLoginPass) {
      res.status(500).send("name is incorrect");
    } else if (
      password != process.env.AdminLoginPass &&
      adminName != process.env.AdminName
    ) {
      res.status(500).send("name and password is incorrect");
    }
  } catch (error) {
